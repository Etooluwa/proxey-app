...
// --- SUB-COMPONENT: INVOICE GENERATOR MODAL ---
...
const InvoiceGeneratorModal = ({ appointment, onClose }: { appointment: typeof UPCOMING_APPOINTMENTS[0], onClose: () => void }) => {
  ...
  const handleUpdateItem = (id: number, field: keyof InvoiceItem, value: string | number) => {
...
const AppointmentDetailView = ({ appointment, onBack }: { appointment: typeof UPCOMING_APPOINTMENTS[0], onBack: () => void }) => {
    ...
    <div className="max-w-5xl mx-auto animate-in fade-in slide-in-from-right-8 duration-300 pb-12 relative">
       ...
       <div className="flex items-center justify-between mb-8">
         ...
         <div className={`px-4 py-1.5 rounded-full text-sm font-bold uppercase tracking-wide border ${
            localStatus === 'CONFIRMED' ? 'bg-green-50 text-green-700 border-green-100' : 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                   ...
                   <div>
                      ...
                      <div className={`flex items-center gap-3 p-4 rounded-2xl border ${
                          localStatus === 'COMPLETED' ? 'bg-emerald-50 border-emerald-100' : 'bg-gray-50 border-gray-100'
                      }`}>
...
export const ProviderSchedule: React.FC = () => {
  ...
  const [eventType, setEventType] = useState<'APPOINTMENT' | 'BLOCK'>('APPOINTMENT');
  const [viewingAppointment, setViewingAppointment] = useState<typeof UPCOMING_APPOINTMENTS[0] | null>(null);
            ...
            <div className="min-w-[300px] md:min-w-full">
                ...
                {DATES.slice(0, 31).map(date => {
                    ...
                    <div 
                        ...
                        className={`h-16 md:h-24 border border-gray-50 rounded-xl p-1 md:p-2 relative cursor-pointer transition-all hover:border-brand-300 flex flex-col justify-between group ${
                        isSelected ? 'bg-brand-50 border-brand-500 shadow-inner' : 'bg-white'
                        }`}
                    >
