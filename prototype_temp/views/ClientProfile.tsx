...
interface ClientProfileProps {
  ...
  onUpdateProfile: (data: UserProfile) => void;
}

export const ClientProfile: React.FC<ClientProfileProps> = ({ profileData, onUpdateProfile }) => {
  const [viewMode, setViewMode] = useState<'MAIN' | 'TRANSACTIONS'>('MAIN');
  ...
  const handleSaveClick = () => {
    onUpdateProfile(tempProfileData);
    setIsEditing(false);
  };
                     ...
                     <tr key={tx.id} className="hover:bg-gray-50/50 transition-colors group">
                       ...
                       <td className="p-6">
                          <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border ${
                            tx.status === 'PAID' ? 'bg-green-50 text-green-600 border-green-100' : 
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <input 
                  ...
                  readOnly={!isEditing} 
                  className={`w-full p-3 rounded-xl text-gray-700 outline-none transition-all ${
                      isEditing ? 'bg-white border border-brand-200 ring-2 ring-brand-50' : 'bg-gray-50 border border-transparent'
                  }`} 
                />
              </div>
              <div>
              ...
              <div className="col-span-2">
                ...
                <textarea 
                   ...
                   readOnly={!isEditing}
                   className={`w-full p-3 rounded-xl text-gray-700 outline-none resize-none h-24 transition-all ${
                       isEditing ? 'bg-white border border-brand-200 ring-2 ring-brand-50' : 'bg-gray-50 border border-transparent'
                   }`}
                />
              </div>
