...
import { BookingStatus, ServiceCategory, Booking } from '../types';
...
type ViewState = 'HOME' | 'ALL_CATEGORIES' | 'CATEGORY_DETAIL';

export const ClientDashboard: React.FC<ClientDashboardProps> = ({ bookings, onProviderClick, onNavigate }) => {
                    ...
                    <div key={booking.id} className="relative pb-6 border-b border-gray-100 last:border-0 last:pb-0">
                      ...
                      {booking.status === BookingStatus.CONFIRMED && (
                        <div className="mt-4 flex gap-2">
                          <button onClick={() => onNavigate('messages')} className="flex-1 py-2 text-xs font-semibold text-brand-600 bg-brand-50 rounded-lg hover:bg-brand-100 transition-colors">
                            Message
                          </button>
                          <button onClick={() => onNavigate('bookings')} className="flex-1 py-2 text-xs font-semibold text-gray-600 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                            Reschedule
                          </button>
                        </div>
                      )}
