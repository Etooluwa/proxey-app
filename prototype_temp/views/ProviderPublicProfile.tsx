...
// Calendar Logic (Duplicated here for self-containment in this file context, or could be imported)
const generateCalendarLinks = (serviceName: string, providerName: string, dateStr: string, location: string) => {
    // Parse string "Oct 27, 2023" + time
    // This is a mock parser. In production, use proper ISO strings.
...
interface BookingSummaryViewProps {
  ...
  provider: typeof TOP_PROVIDERS[0];
  service: typeof PROVIDER_SERVICES[0];
  timeLabel: string;
}

const BookingSummaryView: React.FC<BookingSummaryViewProps> = ({ onBack, onSuccess, onBook, provider, service, timeLabel }) => {
...
interface RequestTimeViewProps {
  onBack: () => void;
  providerName: string;
}

const RequestTimeView: React.FC<RequestTimeViewProps> = ({ onBack, providerName }) => {
...
const CalendarView: React.FC<CalendarViewProps> = ({ onBack, onSelectSlot, onRequestTime, providerName }) => {
  const [selectedDate, setSelectedDate] = useState<number>(27);
  const [selectedTime, setSelectedTime] = useState<string | null>(null);

  const getSlots = (date: number) => {
    if (date < 24) return []; 
    return date % 2 === 0 
             ...
             {slots.length > 0 ? (
                <div className="grid grid-cols-2 gap-3 overflow-y-auto custom-scrollbar pr-2 -mr-2 pb-4">
                    ...
                    <button
                        ...
                        className={`py-3 md:py-4 px-3 rounded-xl text-sm font-bold border transition-all flex items-center justify-center gap-2 ${
                            selectedTime === slot 
...
export const ProviderPublicProfile: React.FC<ProviderPublicProfileProps> = ({ providerId, onBack, onBook }) => {
  const [viewMode, setViewMode] = useState<'PROFILE' | 'CALENDAR' | 'REQUEST_TIME' | 'BOOKING_SUMMARY'>('PROFILE');
  ...
  const displayServices = availableServices.length > 0 ? availableServices : [
      { id: 'default', title: `Standard ${provider.categories[0]} Service`, price: provider.hourlyRate, priceUnit: 'hour' as const, duration: '1h', description: 'Standard service booking', category: provider.categories[0], active: true, bookingsCount: 0 }
  ];
  ...
  const handleCalendarSelection = (timeLabel: string) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          ...
          <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm">
                ...
                <div key={review.id} className="border-b border-gray-50 last:border-0 pb-6 last:pb-0">
                   <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex text-yellow-400 gap-0.5">
                        {[...Array(5)].map((_, i) => (
                          <Icons.Star key={i} size={12} fill={i < review.rating ? "currentColor" : "none"} className={i < review.rating ? "" : "text-gray-300"} />
                        ))}
                      </div>
                   </div>
            ...
            <button className="w-full mt-6 py-3 border border-gray-200 rounded-xl text-gray-600 font-bold text-sm hover:bg-gray-50 transition-colors">
              Show more reviews
            </button>
          </div>
        ...
        <div className="lg:col-span-1 order-first lg:order-last">
          <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-lg lg:sticky lg:top-24">
             ...
             <div className="space-y-6 mb-6">
                   ...
                   <div className="space-y-2 max-h-[240px] overflow-y-auto pr-1 custom-scrollbar">
                     {displayServices.map(s => (
                       <div 
                         ...
                         className={`p-3 rounded-xl border cursor-pointer transition-all flex justify-between items-center group ${selectedService === s.id ? 'border-brand-500 bg-brand-50 ring-1 ring-brand-500' : 'border-gray-200 hover:border-brand-200 hover:bg-gray-50'}`}
                       >
                          <div className="flex-1 min-w-0 mr-2">
                             <div className="flex justify-between items-center mb-1">
                                 <p className={`text-sm font-bold truncate ${selectedService === s.id ? 'text-brand-900' : 'text-gray-700'}`}>{s.title}</p>
                                 <span className="text-xs font-bold text-gray-900 ml-2">${s.price}{s.priceUnit === 'hour' ? '/hr' : ''}</span>
                             </div>
                             <p className="text-xs text-gray-500 line-clamp-1">{s.description}</p>
                             <div className="flex items-center gap-1 mt-1 text-[10px] text-gray-400">
                                <Icons.Clock size={10} /> {s.duration}
                             </div>
                          </div>
                          <div className={`w-4 h-4 rounded-full border flex items-center justify-center flex-shrink-0 ${selectedService === s.id ? 'border-brand-500' : 'border-gray-300'}`}>
                             {selectedService === s.id && <div className="w-2 h-2 bg-brand-500 rounded-full"></div>}
                          </div>
                       </div>
                     ))}
                   </div>
                ...
                <div>
                   <div className="flex justify-between items-center mb-2">
                      <label className="block text-xs font-bold text-gray-500 uppercase">Availability</label>
                      <button 
                   ...
                   <div className="space-y-2">
                      {EXTENDED_DATA.availability.map(slot => (
                        <button
                            key={slot.id}
                            onClick={() => setSelectedTimeLabel(slot.label)}
                            className={`w-full p-3 rounded-xl border text-sm font-medium transition-all flex items-center justify-between group ${
                        ...
                        >
                            <div className="flex items-center gap-2">
                                <Icons.Calendar size={16} className={selectedTimeLabel === slot.label ? 'text-brand-400' : 'text-gray-400 group-hover:text-brand-500'} />
                                <span>{slot.label}</span>
                            </div>
                            {selectedTimeLabel === slot.label && <Icons.Check size={16} className="text-brand-400" />}
                        </button>
                      ))}

                      {selectedTimeLabel && !EXTENDED_DATA.availability.some(s => s.label === selectedTimeLabel) && (
                           <button
                           className="w-full p-3 rounded-xl border text-sm font-medium transition-all flex items-center justify-between bg-brand-600 text-white border-brand-600 shadow-md"
                           >
                                <div className="flex items-center gap-2">
                                    <Icons.Calendar size={16} className="text-brand-200" />
                                    <span>{selectedTimeLabel}</span>
                                </div>
                                <Icons.Check size={16} className="text-brand-200" />
                           </button>
                      )}

                      <button 
                         onClick={() => setViewMode('REQUEST_TIME')}
                         className="w-full pt-2 text-xs text-gray-400 font-medium hover:text-brand-600 transition-colors"
                      >
                         Request custom time
                      </button>
                   </div>
             ...
             <div className="space-y-3">
               <button 
                 ...
                 className="w-full py-4 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl shadow-xl shadow-brand-200 transition-all flex items-center justify-center gap-2 text-lg"
               >
                 {selectedTimeLabel ? 'Book Appointment' : 'Check Availability & Book'}
               </button>
               <button className="w-full py-3 bg-white border-2 border-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-50 hover:border-gray-200 transition-colors flex items-center justify-center gap-2">
                 <Icons.Message size={18} /> Message {firstName}
               </button>
             </div>

             <div className="mt-6 pt-4 border-t border-gray-100 text-center">
                <p className="text-xs text-gray-400 flex items-center justify-center gap-1">
                   <Icons.Shield size={12} /> 100% Satisfaction Guaranteed
                </p>
             </div>
          </div>
        </div>
