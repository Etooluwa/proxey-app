...
export const ProviderMessages: React.FC = () => {
  const [activeChat, setActiveChat] = useState(MOCK_CLIENT_CONVERSATIONS[0]);
  const [showMobileChat, setShowMobileChat] = useState(false);

  const handleChatSelect = (chat: typeof MOCK_CLIENT_CONVERSATIONS[0]) => {
    setActiveChat(chat);
        ...
        <div className="flex-1 overflow-y-auto">
          {MOCK_CLIENT_CONVERSATIONS.map((chat) => (
            <div 
              ...
              className={`p-4 flex gap-3 cursor-pointer border-l-4 transition-all hover:bg-gray-50 ${
